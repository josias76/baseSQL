...
-- Les fonctions d'agrégation en SQL sont utilisées pour effectuer des calculs sur un ensemble de valeurs dans une colonne. 
-- Elles sont couramment utilisées pour obtenir des statistiques résumées à partir des données.
...
-- En combinaison avec la clause `GROUP BY`, vous pouvez utiliser des fonctions d'agrégation telles que SUM(), COUNT(), AVG(), MIN(), MAX(), etc., 
-- pour effectuer des calculs sur les groupes de données résultants. Cela vous permet d'obtenir des statistiques résumées par groupe.
...
-- Les jointures en SQL sont utilisées pour combiner des données de deux ou plusieurs tables en fonction d'une relation entre les colonnes de ces tables. 
-- Elles permettent de récupérer des informations provenant de plusieurs tables dans une seule requête.

/*======================================================
Jointure Interne (INNER JOIN) avec USING
======================================================*/

-- La jointure interne combine les lignes de deux tables en utilisant une condition de correspondance spécifiée, 
-- et seules les lignes qui satisfont cette condition sont incluses dans le résultat. La clause USING permet de spécifier la colonne de correspondance commune.
